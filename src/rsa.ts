...
export class RSA {
    ...
    *  @returns [public key, private key]
     */
    static async generateKeyPair(): Promise<[string, string]> {
     ...
     */
    static async encrypt(publicKey: string, plaintext: string): Promise<string> {
     ...
     */
    static async decrypt(privateKey: string, ciphertextBase64: string): Promise<string> {
     ...
     */
    static async sign(privateKey: string, plaintext: string): Promise<string> {
        const encoder = new TextEncoder();
        const data = encoder.encode(plaintext);
        const importedKey = await RSA.importPrivateKey(privateKey);
